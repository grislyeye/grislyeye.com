...
const monsterStatisticsByCrTable = [
  {
    cr: '0',
    ac: range(0, 13),
    hpRange: range(1, 6),
...
const Nothing = Object.assign(new Maybe(), {
...
const calculateXpFromCr = cr => xpByCrTable[cr];
...
const armourClassAdjustmentCr = (maybeHpCr, ac) => maybeHpCr.map(hpCr => {
  ...
  return Math.trunc((ac - expectedAc) / 2)
});
...
const attackBonusAdjustmentCr = (maybeDamageCr, attackBonus) => maybeDamageCr.map(damageCr => {
  ...
  return Math.trunc((attackBonus - expectedAb) / 2)
});
